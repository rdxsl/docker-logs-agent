...
statusCode=$(curl --write-out %{http_code} --silent --output /dev/null --key conf/production/certs/client.key --cert conf/production/certs/client.pem -k -X GET "https://localhost:7002/v1/containers/docker-logs-agent-test-${VERSION}/logs/?tail=5" -H  "accept: application/json")
if [ $statusCode != 200 ]; then
...
statusCode=$(curl --write-out %{http_code} --silent --output /dev/null --key conf/production/certs/bad-client.key --cert conf/production/certs/client.pem -k -X GET "https://localhost:7002/v1/containers/docker-logs-agent-test-${VERSION}/logs/?tail=5" -H  "accept: application/json")
